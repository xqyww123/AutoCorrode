  ...
  private def runBatch(view: org.gjt.sp.jedit.View, prompt: String, resultFile: String): Unit = {
    ...
    while (!IQAvailable.isAvailable) {
      if (System.currentTimeMillis() > deadlineMs) {
        ...
        writeResult(resultFile, s"ERROR: $msg")
        return  // ok: local return from private method
      }
      Thread.sleep(2000)
    }
    ...
    try {
      ...
      case ex: Exception =>
        ...
        Output.writeln(s"[Batch] ${OpenAIAdapter.usageSummary}")
        val resultJson = usageJson("error", OpenAIAdapter.jsonStr(ex.getMessage), elapsed)
