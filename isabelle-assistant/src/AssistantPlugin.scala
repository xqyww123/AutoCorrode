...
class AssistantPlugin extends EBPlugin {
  ...
  private def runBatch(view: org.gjt.sp.jedit.View, prompt: String, resultFile: String): Unit = {
    ...
    try {
      ...
      val deadlineMs = System.currentTimeMillis() + 300000L // 5 min max

      // Phase 1: wait for I/Q server
      Output.writeln("[Batch] Waiting for I/Q server...")
      while (!IQAvailable.isAvailable) {
        if (System.currentTimeMillis() > deadlineMs) {
          val msg = "TIMEOUT: I/Q server not available after 5 minutes"
          Output.error_message(s"[Batch] $msg")
          writeResult(resultFile, usageJson("error", OpenAIAdapter.jsonStr(msg), 300000L))
          return
        }
        Thread.sleep(2000)
      }
      ...
      while (!theoryReady) {
        if (System.currentTimeMillis() > deadlineMs) {
          val msg = "TIMEOUT: No theory file loaded in PIDE after 5 minutes"
          Output.error_message(s"[Batch] $msg")
          writeResult(resultFile, usageJson("error", OpenAIAdapter.jsonStr(msg), 300000L))
          return
        }
